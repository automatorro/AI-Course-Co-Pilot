...
3. Unifică AI Studio la Data URL (done) și păstrează upload la „Save”.
4. Extinde panelul „Insert Image” cu previzualizare live (done).
5. Adaugă Edge Functions `image-search` și `fetch-image` (documente + endpoints). 
6. Testează fluxuri: local, internet (hotlink/proxy), AI, apoi „Save” pentru persistență.
