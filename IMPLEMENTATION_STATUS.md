...
- **F2-T2** [IN_PROGRESS] Sterilizarea șabloanelor cu headere/etichete hardcodate în română, indiferent de `{{language}}`. **Verificat 2026-08-08 că bug-ul e live**: `WORKBOOK_PROMPT`, `MANUAL_PROMPT`, `EXERCISES_PROMPT`, `VIDEO_SCRIPT_PROMPT` sunt toate atinse prin `handleGoldenStep` → `generateLessonContent`/generare per-modul (calea implicită, `contractPipeline: true`) — deci NU sunt cod mort. Corectat azi (commit separat): toate headerele/etichetele de secțiune și tabel (`Instrucțiuni Participant`→`Participant Instructions`, `Spațiu de Lucru`→`Workspace`, `Rezultat`→`Result`, tabelul de timing din `MANUAL_PROMPT`, scenele din `VIDEO_SCRIPT_PROMPT` etc.) + regulă explicită de traducere adăugată la fiecare prompt. `SLIDES_PROMPT` era deja curat (fără hardcodări RO). Rămas TODO: `generateCostZeroSlides`/`COST_ZERO_SLIDES_LABELS`, prompturile globale (structure/blueprint/course_dna etc.), și inventarul complet cerut de F2-T1 pentru migrația spre `{{label_*}}`.
...
**Concluzie.** Bug-ul de limbă din F2-T2 e real și afectează cursuri active. Corectat azi. `GoldenModuleData`/`GOLDEN_MASTER_PROMPT`/`renderWorkbookSection` rămân de investigat separat (folosite undeva? moarte? candidate pentru F10-T1) — nu s-a atins nimic din ele azi.
...
| 2026-08-08 | S04 | F2-T2 IN_PROGRESS | Pornit F2 (următorul pas logic după reconciliere). Verificat rutare Golden/Legacy (§D-011): `EXERCISES_PROMPT`/`WORKBOOK_PROMPT`/`MANUAL_PROMPT`/`VIDEO_SCRIPT_PROMPT` sunt calea vie sub `contractPipeline: true`, nu cod mort. Corectat headerele/etichetele hardcodate în română din toate cele 4 (Manual era cel mai afectat); `SLIDES_PROMPT` era deja curat. Adăugată regulă explicită de traducere a etichetelor la fiecare prompt. Nu s-a putut rula typecheck local (fără Node/npm, D-005) — verificare făcută prin citire atentă + diff linie-cu-linie (55 inserții/55 ștergeri, fără backtick-uri noi, fără drift de linii). Rămas: `generateCostZeroSlides`, prompturile globale, inventarul F2-T1, F2-T3/T4/T5. |
