    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="card-premium w-full max-w-2xl transform transition-all max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-6 border-b dark:border-hairline">
          ...
          <button onClick={onClose} disabled={isSubmitting} className="p-1 rounded-full hover:bg-paper-alt interactive-soft">
            <X size={24} />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="overflow-y-auto p-6 space-y-4 flex-1 custom-scrollbar">
            <div>
              <label htmlFor="title" className="block text-sm font-medium text-graphite">{t('modal.newCourse.courseTitle')}</label>
              <input type="text" id="title" value={title} onChange={e => setTitle(e.target.value)} required className="mt-1 input-premium" />
            </div>
            <div>
              <label htmlFor="subject" className="block text-sm font-medium text-graphite">{t('modal.newCourse.subject')}</label>
              <input type="text" id="subject" value={subject} onChange={e => setSubject(e.target.value)} required className="mt-1 input-premium" />
            </div>
            <div>
              <label htmlFor="audience" className="block text-sm font-medium text-graphite">{t('modal.newCourse.targetAudience')}</label>
            ...
            <div className="bg-paper-alt dark:bg-paper-alt border border-hairline dark:border-hairline rounded-lg p-4">
              <label htmlFor="learning_objectives" className="flex items-center gap-2 text-sm font-medium text-graphite mb-2">
                <Lightbulb size={18} className="text-gold" />
                {t('modal.newCourse.learningObjectives')}
                <span className="text-xs text-stone font-normal">(Optional, but recommended)</span>
              </label>
              <textarea
              ...
              />
              <p className="text-xs text-stone mt-2 flex items-start gap-1">
                <span>💡</span>
            ...
            <div className="space-y-3">
              <label className="block text-sm font-medium text-graphite">{t('modal.newCourse.environment')}</label>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div
                  onClick={() => setEnvironment(GenerationEnvironment.LiveWorkshop)}
                  className={`cursor-pointer p-4 rounded-xl border-2 transition-all flex flex-col items-center text-center gap-2 ${environment === GenerationEnvironment.LiveWorkshop ? 'border-gold bg-paper-alt dark:bg-paper-alt' : 'border-gray-200 dark:border-gray-700 hover:border-hairline'}`}
                >
                  <Presentation size={32} className={environment === GenerationEnvironment.LiveWorkshop ? 'text-gold' : 'text-gray-400'} />
                  <div>
                    <div className="font-bold text-graphite">{t('modal.newCourse.environment.liveworkshop')}</div>
                    <div className="text-xs text-stone mt-1">Focus: Slides, Manual, Interaction</div>
                  </div>
                </div>

                <div
                  onClick={() => setEnvironment(GenerationEnvironment.OnlineCourse)}
                  className={`cursor-pointer p-4 rounded-xl border-2 transition-all flex flex-col items-center text-center gap-2 ${environment === GenerationEnvironment.OnlineCourse ? 'border-gold bg-paper-alt dark:bg-paper-alt' : 'border-gray-200 dark:border-gray-700 hover:border-hairline'}`}
                >
                  <MonitorPlay size={32} className={environment === GenerationEnvironment.OnlineCourse ? 'text-gold' : 'text-gray-400'} />
                  <div>
                    <div className="font-bold text-graphite">{t('modal.newCourse.environment.onlinecourse')}</div>
                    <div className="text-xs text-stone mt-1">Focus: Video Scripts, Cheat Sheets</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <label htmlFor="language" className="block text-sm font-medium text-graphite">{t('modal.newCourse.language')}</label>
